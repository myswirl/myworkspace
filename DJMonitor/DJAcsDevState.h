...
DES_FAX_SENDFAX_START              = (DES_FAX_START | 0x11), // 开始传真
DES_FAX_SENDFAX_SETLOCAL           = (DES_FAX_START | 0x12), // 传真格式设定
DES_FAX_SENDFAX_RESOLVE_FISNIESH   = (DES_FAX_START | 0x13), // 传真分解完成
DES_FAX_SENDFAX_LOCAL_FINISH       = (DES_FAX_START | 0x14), //
DES_FAX_SENDFAX_SHAKEHAND_END      = (DES_FAX_START | 0x15), // 握手信号结束
DES_FAX_SENDFAX_SENDDATA           = (DES_FAX_START | 0x16), // 设置传真页，发送数据
DES_FAX_SENDFAX_SENDFINISH         = (DES_FAX_START | 0x17), // 数据发送完毕
DES_FAX_SENDFAX_USERSTOP           = (DES_FAX_START | 0x18), // 停止传送传真
DES_FAX_SENDFAX_TIFFFILE_NOTEXIST  = (DES_FAX_START | 0x19), // 文件不存在
DES_FAX_SENDFAX_SENDEND            = (DES_FAX_START | 0x20), // 发送结束
DES_FAX_SENDFAX_SENDSTOP           = (DES_FAX_START | 0x21), //

DES_FAX_RECVFAX_RECVCMD            = (DES_FAX_START | 0x30), // 接收传真
DES_FAX_RECVFAX_WAIT_HANDSHAKE     = (DES_FAX_START | 0x31), // 等待握手
DES_FAX_RECVFAX_HANDSHAKE_OK       = (DES_FAX_START | 0x32), // 握手成功
DES_FAX_RECVFAX_RECVDATA_START     = (DES_FAX_START | 0x33), // 页后数据成功，开始接收数据
DES_FAX_RECVFAX_BACKPAGESTATE_SUCC = (DES_FAX_START | 0x34), // 页后状态成功
DES_FAX_RECVFAX_RE_HANDSHAKE       = (DES_FAX_START | 0x35), // 重握手
DES_FAX_RECVFAX_RECVDATA_END       = (DES_FAX_START | 0x36), // 接收停止
DES_FAX_RECVFAX_USERSTOP           = (DES_FAX_START | 0x37), // StopFax
DES_FAX_RECVFAX_COMPOSE_START      = (DES_FAX_START | 0x38), // 合成开始
DES_FAX_RECVFAX_COMPOSE_END        = (DES_FAX_START | 0x39), // 合成结束
DES_FAX_RECVFAX_BACKPAGESTATE_FAIL = (DES_FAX_START | 0x40), // 页后状态失败
DES_FAX_RECVFAX_RECVEND            = (DES_FAX_START | 0x41), // 接收结束
};
