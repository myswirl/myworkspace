...
//#if 0
	printk(KERN_DEBUG "complete: complete_cleanup \n");
...
//#if 0
	...
	if (complete_major) {
		...
		printk(KERN_WARNING "complete: register_chrdev_region, major=%d \n",complete_major);
	} else {
		result = alloc_chrdev_region(&dev, complete_minor , 1,"complete");
		complete_major = MAJOR(dev);
		printk(KERN_WARNING "complete: alloc_chrdev_region, major=%d \n",complete_major);
	}
	if (result < 0) {
