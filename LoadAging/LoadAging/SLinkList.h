...
public:
...
#ifdef WIN32
		CRITICAL_SECTION m_csAddDel;					//插入删除单链表加锁，互斥，多线程中使用此数据结构
#endif        
        ...
        void print();//打印节点
		void printAllData(char *str);						//打印所有数据
        ~SLinkList();//析够函数
};
...
SLinkList<T>::SLinkList(T& n):length(0){
		...
		InitializeCriticalSection( &m_csAddDel );
...
{
	EnterCriticalSection(&m_csAddDel);
        SLNode<T> *p = new SLNode<T>(n);
        tail->next = p;
        tail = p;
        p->next =NULL;
        length++;
	LeaveCriticalSection( &m_csAddDel );
}
//清空
template<typename T>
...
void SLinkList<T>::print(){
	int i=1;
	cout << "Head:" << head->data << " Length:" << length << " ";
...
//实现所有节点数据
template<typename T>
void SLinkList<T>::printAllData(char *str){
    ...
    for(SLNode<T> *p = head; p!=tail; p=p->next)
    {
		sprintf(str,"%s %02X",str, p->next->data);
    }        
}
//析构函数
template<typename T>
