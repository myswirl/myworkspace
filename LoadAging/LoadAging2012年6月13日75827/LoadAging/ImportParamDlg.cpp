...
BOOL ImportParamDlg::OnInitDialog() 
{
	...
	}else 
	{
		...
		if (g_AllCar[selTab].testParam.layerParam[selLayerTab].paraMode == 0)
		{
			this->SetDlgItemText( IDC_EDIT_PARAMODE,"无并联" );
			
		}else if(g_AllCar[selTab].testParam.layerParam[selLayerTab].paraMode == 1)
		{
			this->SetDlgItemText( IDC_EDIT_PARAMODE,"两路并联" );
			
		}else if(g_AllCar[selTab].testParam.layerParam[selLayerTab].paraMode == 2)
		{
			this->SetDlgItemText( IDC_EDIT_PARAMODE,"四路并联" );
		}	
...
void ImportParamDlg::OnButtonLoadparam() 
{
	...
	{
		this->SetDlgItemText( IDC_EDIT_PARAMSETMODE,"整区设置");
		GetDlgItem(IDC_TAB_ALLLAYER)->ShowWindow(FALSE);
		selLayerTab =6;

	}else if (readSS.paramSetMode == 1)
	{
		this->SetDlgItemText( IDC_EDIT_PARAMSETMODE,"分层设置");
		GetDlgItem(IDC_TAB_ALLLAYER)->ShowWindow(TRUE);
		selLayerTab =0;
	}	
	...
	if (readSS.layerParam[selLayerTab].paraMode == 0)
	{
		this->SetDlgItemText( IDC_EDIT_PARAMODE,"无并联" );

	}else if(readSS.layerParam[selLayerTab].paraMode == 1)
	{
		this->SetDlgItemText( IDC_EDIT_PARAMODE,"两路并联" );

	}else if(readSS.layerParam[selLayerTab].paraMode == 2)
	{
		this->SetDlgItemText( IDC_EDIT_PARAMODE,"四路并联" );
	}		
...
void ImportParamDlg::OnSelchangeTabAllcar(NMHDR* pNMHDR, LRESULT* pResult) 
{
	...
	if(strlen(g_AllCar[selTab].paramFileName) == 0)		//未进行参数导入
	{
		...
		((CButton *)this->GetDlgItem (IDC_CHECK_SAVE_RECDATA))->SetCheck (false);
		this->SetDlgItemText( IDC_EDIT_LOADMODE,"" );
		this->SetDlgItemText( IDC_EDIT_PARAMODE,"" );
		//电压、电流固定值
		this->GetDlgItem(IDC_EDIT_LAYER1_CHN1_VALUE)->SetWindowText( "");
		this->GetDlgItem(IDC_EDIT_LAYER1_CHN2_VALUE)->SetWindowText( "");
		this->GetDlgItem(IDC_EDIT_LAYER1_CHN3_VALUE)->SetWindowText( "");
...
void ImportParamDlg::OnOK() 
{
	...
	if (readSS.paramSetMode == 0)//整区设置
	{
		for (int loadIndex=readSS.startLoadNum; loadIndex<=readSS.endLoadNum; loadIndex++)
		{
			g_AllCar[selTab].m_Load[loadIndex-1].m_LoadMode = readSS.layerParam[6].loadMode;//第7层参数赋值给每个通道
			g_AllCar[selTab].m_Load[loadIndex-1].m_ParaMode = readSS.layerParam[6].paraMode;//第7层参数赋值给每个通道
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[0].m_SetValue=readSS.layerParam[6].chn1Value;
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[0].m_SetMax=readSS.layerParam[6].chn1Max;
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[0].m_SetMin=readSS.layerParam[6].chn1Min;
			strcpy(g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[0].m_SetProName,readSS.layerParam[6].chn1ProName);
			
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[1].m_SetValue=readSS.layerParam[6].chn2Value;
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[1].m_SetMax=readSS.layerParam[6].chn2Max;
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[1].m_SetMin=readSS.layerParam[6].chn2Min;
			strcpy(g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[1].m_SetProName,readSS.layerParam[6].chn2ProName);
			
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[2].m_SetValue=readSS.layerParam[6].chn3Value;
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[2].m_SetMax=readSS.layerParam[6].chn3Max;
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[2].m_SetMin=readSS.layerParam[6].chn3Min;
			strcpy(g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[2].m_SetProName,readSS.layerParam[6].chn3ProName);
			
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[3].m_SetValue=readSS.layerParam[6].chn4Value;
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[3].m_SetMax=readSS.layerParam[6].chn4Max;
			g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[3].m_SetMin=readSS.layerParam[6].chn4Min;
			strcpy(g_AllCar[selTab].m_Load[loadIndex-1].m_Channel[3].m_SetProName,readSS.layerParam[6].chn4ProName);
	...
	}else if (readSS.paramSetMode == 1)//分层设置
	{
		for (int loadIndex=readSS.startLoadNum; loadIndex<=readSS.endLoadNum; loadIndex++)//全局负载参数设置，根据全局负载所属的层，分别对通道参数赋值
		{
			...
			g_AllCar[selTab].m_Load[loadIndex-1].m_ParaMode = readSS.layerParam[layerID].paraMode;
	...
	for (int loadInde=readSS.startLoadNum; loadInde<=readSS.endLoadNum; loadInde++)//全局负载参数设置，根据全局负载所属的层，分别对通道参数赋值
	{
