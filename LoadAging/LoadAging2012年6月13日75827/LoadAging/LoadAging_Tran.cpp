...
char			cfg_SoftwareVersion[32]="V1.04";					//软件版本标识
...
int				cfg_DialNumMethod;							//发送命令，拨码策略
//系统全局变量--------------------------------------------
...
void	ReadFromConfig(void)
{
	...
	cfg_DialNumMethod = GetPrivateProfileInt ( "ConfigInfo", "DialNumMethod", 2, cfg_IniName);	
...
void WriteSerialPortCommand(int carID, int serialPortCommandType, int curLoadNum)
{
	...
	if (serialPortCommandType ==LOAD_COMMAND_SET || serialPortCommandType == LOAD_COMMAND_READ || serialPortCommandType == LOAD_COMMAND_STOP)
	{
		if (curLoadNum==0)
		{
			curLoadNum=63;
		...
		}else if(cfg_DialNumMethod ==2)
		{
			if(curLoadNum>=1 && curLoadNum<=60)
			{
				
				wBuffer[1] = curLoadNum+70;
			}
			
		}else if(cfg_DialNumMethod ==3)
		{
			if(curLoadNum>=1 && curLoadNum<=60)
			{
				wBuffer[1] = curLoadNum*2+130;
			}
		}
...
void	DealWithSerialPortData(int carID)				
{
	...
	//64是所有, 主控板63, 
	if(cfg_DialNumMethod==2 && loadNum>=71 && loadNum<=130)
	{
		loadNum=loadNum-70;
	}
	if(cfg_DialNumMethod==3 && loadNum>130 && loadNum<=250)
	{
		loadNum=(loadNum-130)/2;
	}
