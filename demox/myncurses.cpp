...
void mainWork( void )
{
	...
	init_win_params( &g_win_head, WIN_HEIGHT, WIN_WIDTH, 2, 0 );
	create_box( &g_win_head, TRUE );

	for( i = 0; i < TRK_NUM_PERPAGE; i++ )
	{
		init_win_params( &g_win_Trk[i], WIN_HEIGHT, WIN_WIDTH, 4 + i * 2, 0 );
		create_box( &g_win_Trk[i], TRUE );
	}
	sprintf( tmpStr, "%-55s(X:%d,Y:%d)  (%02d/%02d)", "BankDemo,'q':Exit,'Up':previous page,'Down':Next page.", COLS, LINES, g_cur_page, g_total_page_num );
...
void draw_main( TRUNK_STRUCT* pOneTrunk )
{
	...
	sprintf( tmpStr, "%03d (%02d,%03d) %-7s %-12s %-9s %-7s %-7s %-7s (%02d,%03d)",
	         ...
	         GetString_TrunkState( pOneTrunk->State ), pOneTrunk->CalleeCode, pOneTrunk->CallerCode, pOneTrunk->DtmfBuf,
