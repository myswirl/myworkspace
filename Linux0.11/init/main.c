...
#include <unistd.h>                                                                 // 标准符号常数与类型文件。定义了各种符号常数和类型，并申明了各种函数。
 ...
 * until an execve is executed. This is no problem, but for the stack. This is handled by
 * not letting main() use the stack at all after fork().
 * Thus, no function calls - which means inline code for fork too, as otherwise we
 * would use the stack upon exit from 'fork()'.
 *
 * Actually only pause and fork are needed inline, so that there won't be any messing with
 * the stack from main(), but we define some others too.
 *
...
#include <stdarg.h>                                                                 /* 标准参数头文件。以宏的形式定义变量参数列表。主要说明了-个
                                                                                       类型(va_list)和三个宏(va_start, va_arg 和va_end)，vsprintf、vprintf、vfprintf。*/
#include <unistd.h>
 ...
 */
#define EXT_MEM_K		( *(unsigned short *)0x90002 )                              // 1M 以后的扩展内存大小KB。
...
void main( void )
{
    /* 这里确实是void，并没错。在startup 程序(head.s)中就是这样假设的,参见head.s 程序第136 行开始的几行代码。
     *
     * 此时中断仍被禁止着，做完必要的设置后就将其开启。
     *
     * 下面这段代码用于保存：
     * 根设备号ORIG_ROOT_DEV,高速缓存末端地址??buffer_memory_end,
...
* Output:         //
* Return:         //
* Others:         // 产生格式化信息并输出到标准输出设备stdout(1)，这里是指屏幕上显示。参数'*fmt'指定输出将
